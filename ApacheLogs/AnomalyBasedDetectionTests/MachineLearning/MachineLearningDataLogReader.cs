...
namespace LogAnalysisTool.ApacheLogs.AnomalyBasedDetectionTests.MachineLearning
{
        ...
        public static IEnumerable<T> GetData(string dataFileName, Regex regexForLogFile, Func<Match, int, T> CreateDataItem)
        {
            ...
            using (var dataReader = new StreamReader(dataFileName))
            {
                ...
                while (!dataReader.EndOfStream)
                {
                    ...
                    if (!m.Success)
                    {
                        throw new Exception($"Apache Log File Regex is not a match for a the log line {dataLine}");
                    }
